  ...
  <div class="container">
    ...
    <div class="leaderboard-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <p class="page-subtitle">Top performers in our learning community</p>
        </div>
        <div style="display: flex; gap: 0.75rem; align-items: center;">
          <a href="{{ route('points.index') }}" style="background: linear-gradient(135deg, var(--cyan-accent), var(--purple-accent)); color: white; padding: 0.6rem 1rem; border-radius: 8px; text-decoration: none; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(0, 201, 255, 0.3)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'">
            <i class="fas fa-shopping-cart"></i>Redeem Points
          </a>
          <button onclick="openPointsModal()" class="info-btn">
      ...
      <div class="filters-section">
        <div class="filter-tabs">
          <a href="?period=7" class="filter-tab {{ ($period ?? 'all') === '7' ? 'active' : '' }}">7 days</a>
          <a href="?period=30" class="filter-tab {{ ($period ?? 'all') === '30' ? 'active' : '' }}">30 days</a>
          <a href="?period=all" class="filter-tab {{ ($period ?? 'all') === 'all' ? 'active' : '' }}">All time</a>
        </div>
      ...
      <div class="stats-card">
        <div class="stats-left">
          <div class="my-avatar">
            ...
            @php
              ...
              $userLevel = 1;
              if ($userPoints >= 10000) $userLevel = 10;
              elseif ($userPoints >= 7500) $userLevel = 9;
              elseif ($userPoints >= 5000) $userLevel = 8;
              elseif ($userPoints >= 3500) $userLevel = 7;
              elseif ($userPoints >= 2000) $userLevel = 6;
              elseif ($userPoints >= 1000) $userLevel = 5;
              elseif ($userPoints >= 500) $userLevel = 4;
              elseif ($userPoints >= 250) $userLevel = 3;
              elseif ($userPoints >= 100) $userLevel = 2;
            @endphp
          ...
          <div class="my-info">
            ...
            <div class="points-info">
              <span class="points">{{ auth()->user()->getTotalPoints() }}</span>
        ...
        <div class="stats-right">
          ...
          <div class="progress-info">
            @if(is_numeric($pointsToNext) && $pointsToNext > 0)
              <span>{{ $pointsToNext }} to next level</span>
            @elseif((auth()->user()->level ?? 1) >= 9)
              <span>Max level!</span>
            @else
    ...
    <div class="leaderboard-list">
      @forelse($topUsers as $index => $user)
        <div class="leader-item {{ $index < 3 ? 'podium' : '' }}">
          <div class="position">
            @if($index === 0)
          ...
          <div class="user-profile">
            <div class="profile-avatar">
              ...
              @php
                $points = $user->gamification_points_sum_points ?? 0;
                $level = 1;
                if ($points >= 10000) $level = 10;
                elseif ($points >= 7500) $level = 9;
                elseif ($points >= 5000) $level = 8;
                elseif ($points >= 3500) $level = 7;
                elseif ($points >= 2000) $level = 6;
                elseif ($points >= 1000) $level = 5;
                elseif ($points >= 500) $level = 4;
                elseif ($points >= 250) $level = 3;
                elseif ($points >= 100) $level = 2;
              @endphp
              <div class="level-indicator">{{ $level }}</div>
            </div>
            <div class="profile-info">
              <h4>{{ $user->name }}</h4>
              <p>{{ $user->bio ?? 'Digital Leap Africa Learner' }}</p>
            </div>
          </div>
          
          <div class="score-section">
            <div class="points-score">{{ $user->gamification_points_sum_points ?? 0 }}</div>
